...
def process_spotify_json_file(filepath, user_id):
    try:
        ...
        for entry in streams:
            try:
                ts = entry["ts"]
                # Normalize timestamp to UTC string format
                ts_dt = pendulum.parse(ts).in_timezone("UTC").to_datetime_string()
            except Exception as ts_err:
                logger.warning(f"[{filepath}] Failed to parse timestamp '{ts}': {ts_err}")
                continue

            logger.info(f"Checking for existing usage_log entry at {ts_dt} for user {user_id}...")
            ...
            if exists:
                logger.info(f"Usage log for {ts_dt} already exists. Skipping.")
                continue
            ...
            conn_country = entry.get("conn_country") or None
            ...
            insert_usage_logs.append((
                user_id, song_id, ts_dt, ms_played, conn_country,
